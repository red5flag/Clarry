use crate::tokens::builders::prelude::*;
use crate::tokens::node::IntoToken;
use crate::data::app_data::{seed_users, seed_posts};

let user = &seed_users()[0];
let posts = seed_posts();

col
        id twitter_page
        css min-h-screen bg-black text-white
        row
            css sticky top-0 z-50 bg-black/80 backdrop-blur border-b border-gray-800 px-4 py-3 items-center
            img_block user.avatar_url.as_str()
                css w-8 h-8 rounded-full mr-4
            txt "Home"
                css text-lg font-bold flex-1
            txt "⚙"
                css text-xl
        row
            css px-4 py-3 border-b border-gray-800 gap-3
            img_block user.avatar_url.as_str()
                css w-10 h-10 rounded-full
            block
                css flex-1 bg-gray-900 rounded-xl px-4 py-2
                txt "What's happening?"
                    css text-gray-500
        col
            css divide-y divide-gray-800
            .add_all(posts.iter().map(|p| tweet_card(p, user)))
        row
            css fixed bottom-0 left-0 right-0 bg-black border-t border-gray-800 py-3 px-8 justify-between
            txt "🏠"
                css text-xl
            txt "🔍"
                css text-xl
            txt "🔔"
                css text-xl
            txt "✉"
                css text-xl
}

fn tweet_card(post: &crate::data::app_data::MediaPost, _me: &crate::data::app_data::UserProfile) -> impl IntoToken {
    row
        css px-4 py-3 gap-3
        img_block post.author_avatar.as_str()
            css w-10 h-10 rounded-full flex-shrink-0
        col
            css flex-1 min-w-0
            row
                css gap-1 items-baseline
                txt post.author_name.as_str()
                    css font-bold text-sm
                txt format!("@{} · {}", post.author_id, post.timestamp)
                    css text-gray-500 text-sm
            txt post.content.as_str()
                css text-sm mt-1
            .add_opt(post.image_url.as_ref().map(|url| {
                img_block(url.as_str()).set_class("w-full rounded-xl mt-2 max-h-64 object-cover")
            }))
            row
                css mt-3 gap-6 text-gray-500
                txt "💬"
                    css text-sm
                txt format!("{}", post.comments)
                    css text-sm
                txt "🔁"
                    css text-sm
                txt "♡"
                    css text-sm
                txt format!("{}", post.likes)
                    css text-sm
                txt "📤"
                    css text-sm

